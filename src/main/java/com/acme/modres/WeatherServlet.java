...
public class WeatherServlet extends HttpServlet {
  ...
  // Azure Key Vault secret name for weather API key
  ...
  /**
   * Retrieve weather API key from Azure Key Vault
   * Replaces hard-coded credentials with secure secret management
   */
  private String getWeatherApiKeyFromKeyVault() {
    try {
      if (secretClient != null) {
        return secretClient.getSecret(WEATHER_API_KEY_SECRET).getValue();
      } else {
        logger.warning("Azure Key Vault client not available, falling back to environment variable");
        return System.getenv("WEATHER_API_KEY");
      }
    } catch (Exception e) {
      logger.warning("Failed to retrieve API key from Key Vault: " + e.getMessage());
