...
public class WeatherServlet extends HttpServlet {
  ...
  // Fallback: local OS environment variable key name (kept for backward compatibility)
  ...
  protected void doGet(HttpServletRequest request,
    ...
    // Retrieve API key from AWS Secrets Manager; fall back to environment variable
   ...
   * Falls back to the WEATHER_API_KEY environment variable if Secrets Manager is unavailable.
   * This replaces any hardcoded credentials in source code.
   */
  private String getWeatherApiKey() {
    // First try AWS Secrets Manager for centralized secret management
    try (SecretsManagerClient secretsClient = SecretsManagerClient.builder().build()) {
      ...
      if (secretValue != null && !secretValue.trim().isEmpty()) {
        logger.fine("Weather API key retrieved from AWS Secrets Manager");
      ...
      logger.warning("Unexpected error accessing AWS Secrets Manager: " + e.getMessage()
          + ". Falling back to environment variable.");
    }
    ...
    if (envKey != null && !envKey.trim().isEmpty()) {
      logger.fine("Weather API key retrieved from environment variable");
      return envKey.trim();
    }
