...
echo === ModResorts - Build and Push Docker Image (Windows) ===

set /p PROJECT_NAME="Enter project name [ModResorts]: "
if "%PROJECT_NAME%"=="" set PROJECT_NAME=ModResorts

for /f "delims=" %%A in ('powershell -NoLogo -Command "$n='%PROJECT_NAME%'.ToLower(); $n -replace '[^a-z0-9]', '-' -replace '^-+', '' -replace '-+$', ''"') do set IMAGE_NAME=%%A
...
for /f "delims=" %%A in ('powershell -NoLogo -Command "$t='%IMAGE_TAG%'.ToLower(); $t -replace '[^a-z0-9]', '-' -replace '^-+', '' -replace '-+$', ''"') do set IMAGE_TAG=%%A
if "%IMAGE_TAG%"=="" set IMAGE_TAG=latest

set FULL_IMAGE_NAME=!REPOSITORY!:!IMAGE_TAG!
