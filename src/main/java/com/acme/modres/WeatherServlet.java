...
public class WeatherServlet extends HttpServlet {
  ...
  public void init() {
    ...
    // Load weather API key from AWS Secrets Manager
    this.cachedWeatherApiKey = getWeatherApiKeyFromSecretsManager();
  ...
  private String getWeatherApiKeyFromSecretsManager() {
    try {
      GetSecretValueRequest getSecretValueRequest = GetSecretValueRequest.builder()
          .secretId(WEATHER_API_KEY_SECRET)
          .build();
      
      GetSecretValueResponse getSecretValueResponse = secretsClient.getSecretValue(getSecretValueRequest);
      String secret = getSecretValueResponse.secretString();
      
      // Parse JSON if needed, or return directly if it's a plain string
      // For simplicity, assuming the secret is stored as plain text
      logger.info("Successfully retrieved weather API key from AWS Secrets Manager");
      return secret;
      
    } catch (Exception e) {
      logger.warning("Failed to retrieve weather API key from AWS Secrets Manager: " + e.getMessage());
