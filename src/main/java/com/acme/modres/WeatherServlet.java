...
public class WeatherServlet extends HttpServlet {
  ...
  private static final String SECRET_PROJECT_ID = System.getenv().getOrDefault("GCP_PROJECT_ID", "default-project");
  private static final String WEATHER_API_KEY_SECRET_NAME = "weather-api-key";
  private static final String WEATHER_API_KEY_SECRET_VERSION = "latest";
  ...
  private String getSecretFromSecretManager() {
    try (SecretManagerServiceClient client = SecretManagerServiceClient.create()) {
      ...
      logger.info("Successfully retrieved secret from Google Secret Manager");
      return secret;
    } catch (Exception e) {
      logger.warning("Failed to retrieve secret from Secret Manager, falling back to environment variable: " + e.getMessage());
